<h1 align="center">K Priya Darshini</h1>
<h3 align="center">Data Analyst | Python | SQL | Power BI</h3>

<p align="center">
Exploratory Data Analysis • Statistical Insights • Dashboard Development • Problem Solving
</p>

---

## 👩‍💼 Professional Summary

Detail-oriented Data Analyst with hands-on experience in Python, SQL, Excel, and Power BI. 
Experienced in performing end-to-end data analysis including data cleaning, exploratory data analysis (EDA), statistical validation, and interactive dashboard development.

Strong analytical mindset with the ability to convert raw datasets into structured insights through visualization and reporting. 
Supported by a solid foundation in Data Structures & Algorithms and practical full-stack development experience.

---

<h3 align="center">Technical Stack</h3>

---

## 🛠️ Technical Competencies

### Programming Languages
<p>
<img src="https://skillicons.dev/icons?i=python" height="40"/>  &nbsp;&nbsp;
<img src="https://skillicons.dev/icons?i=java" height="40"/> &nbsp;&nbsp;
<img src="https://skillicons.dev/icons?i=mysql" height="40"/>  &nbsp;&nbsp;
<img src="https://skillicons.dev/icons?i=dart" height="40"/> 
</p>

---

### Data Analysis & Visualization
<p>
<img src="https://img.shields.io/badge/Pandas-150458?style=flat&logo=pandas&logoColor=white" height="28"/> &nbsp;&nbsp;
<img src="https://img.shields.io/badge/NumPy-013243?style=flat&logo=numpy&logoColor=white" height="28"/> &nbsp;&nbsp;
<img src="https://img.shields.io/badge/Matplotlib-11557C?style=flat" height="28"/> &nbsp;&nbsp;
<img src="https://img.shields.io/badge/Power%20BI-F2C811?style=flat&logo=power-bi&logoColor=black" height="28"/> &nbsp;&nbsp;
<img src="https://img.shields.io/badge/Excel-217346?style=flat&logo=microsoft-excel&logoColor=white" height="28"/> 
</p>

- Data Cleaning & Preprocessing  
- Exploratory Data Analysis (EDA)  
- Statistical Analysis  
- KPI Dashboard Development  
- Data Visualization & Reporting  

---

### Machine Learning (Foundational)
<p>
<img src="https://skillicons.dev/icons?i=tensorflow" height="40"/> 
</p>

- Supervised & Unsupervised Learning  
- Feature Engineering  
- Model Evaluation  

---

### Full-Stack Development (Technical Strength)
<p>
<img src="https://skillicons.dev/icons?i=react" height="40"/>  &nbsp;&nbsp;
<img src="https://skillicons.dev/icons?i=fastapi" height="40"/> &nbsp;&nbsp;
<img src="https://skillicons.dev/icons?i=flutter" height="40"/>  &nbsp;&nbsp;
<img src="https://skillicons.dev/icons?i=firebase" height="40"/> 
</p>

- REST API Development  
- CRUD Operations  
- Authentication & API Integration  
- Backend Data Handling  

---

### Tools & DevOps
<p>
<img src="https://skillicons.dev/icons?i=git" height="40"/> &nbsp;&nbsp;
<img src="https://skillicons.dev/icons?i=github" height="40"/>  &nbsp;&nbsp;
<img src="https://skillicons.dev/icons?i=docker" height="40"/> 
</p>

---

### Data Structures & Algorithms
Arrays • Strings • Linked Lists • Stacks • Queues • Sorting Algorithms • Recursion • Time & Space Complexity Analysis

---

## 🚀 Key Projects

### 📊 End-to-End Data Analysis
- Cleaned and transformed datasets using Pandas and NumPy.
- Conducted exploratory data analysis to identify trends and patterns.
- Applied statistical techniques to validate findings.
- Built clear visualizations to communicate insights effectively.

### 📈 Power BI Dashboard Development
- Designed interactive dashboards to monitor key performance indicators.
- Implemented DAX calculations for dynamic measures.
- Used slicers and filters for enhanced data exploration.
- Structured reports for analytical clarity.

---

## 💻 Application Development Experience

### 🌐 Full-Stack Applications
- Developed web applications using React (Frontend) and FastAPI (Backend).
- Built REST APIs and implemented CRUD operations.
- Integrated authentication systems and structured backend logic.

### 📱 Mobile Applications
- Developed two cross-platform applications using Flutter.
- Integrated third-party APIs for real-time data access.
- Implemented Firebase authentication and state management.
- Designed clean, responsive user interfaces.

---

## 📜 Certifications
- Python for Data Analysis  
- SQL for Data Analytics  
- Power BI Fundamentals  
- Data Visualization with Excel  

---

## 📚 Current Focus
- Advanced SQL query optimization  
- Real-world analytics case studies  
- Strengthening statistical analysis techniques  
- Continuous Data Structures & Algorithms practice  

---

## 📫 Contact

LinkedIn: https://www.linkedin.com/in/k-priya-darshini-07ab932a0  
Email: kpriyadharshinircr@gmail.com  
Portfolio: https://k-priyadarshini-portfolio.vercel.app/

---

Open to Data Analyst and Data Science entry-level opportunities.
